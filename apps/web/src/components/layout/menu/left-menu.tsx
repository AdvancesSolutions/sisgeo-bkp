          ...
          <Box className="h-full w-full">
            <Paper elevation={0} className="h-full w-full rounded-4xl py-6">
              <Box className="relative h-full w-full overflow-x-hidden">
                ...
                >
                  <Box className="mb-2 flex items-center justify-between gap-1 pe-1">
                    {activeItem?.label && (
                      <Typography variant="h6" className="text-primary flex-1 px-2.5">
                        {t(activeItem?.label)}
                      </Typography>
                    )}
                    <Tooltip title={t("menu-minimize")} placement="right" arrow>
                      <IconButton
                        size="small"
                        color="grey"
                        className="icon-only shrink-0"
                        onClick={hideLeftSecondary}
